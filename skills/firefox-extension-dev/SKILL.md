...
- **Cleanest reinstall order (do it in THIS sequence): 1) uninstall/remove the old version, 2) fully restart the browser, 3) install the new version.** Restarting only AFTER the old version is gone guarantees nothing survives in the already-running process — no cached parent module, no lingering observer, no in-memory state. Restarting BEFORE removing is weaker: session restore brings the old add-on back at startup, so the reinstall still fights its leftovers. A plain "Reload" is fine only for content/background/popup/options/CSS edits.
...
- **Always open the test window OUT OF THE WAY — minimize it (or move it to another desktop) right after launch, by default.** It otherwise steals focus, raises itself, and its own Firefox popups float above everything. Treat minimized as the normal mode, not an option (only un-minimize for the rAF/timer caveat below). Two parts:
  1. **Unique window class** so you never touch the user's real Firefox (which shares the default `firefox-default` class): launch with `--class my-test --name my-test` (X11). Then `wmctrl -lx | grep my-test` matches only the test window.
  2. **Move it off-view without stealing focus.** Detect the environment first (`echo $XDG_SESSION_TYPE $XDG_CURRENT_DESKTOP`; `command -v wmctrl xdotool`). On X11/EWMH: if there is more than one virtual desktop (`wmctrl -d | wc -l` > 1), move it with `wmctrl -ir <WID> -t <otherIndex>` (doesn't switch the active desktop); else minimize it with `xdotool windowminimize <WID>` (leaves `_NET_WM_STATE … HIDDEN`, doesn't activate it; RDP keeps working while hidden). Get `<WID>` from `wmctrl -lx | grep my-test | awk '{print $1}'`. Don't create extra desktops unless asked. Wayland has no portable client-side mover — just minimize if a tool exists, else leave it. This is a nicety, not correctness — don't hard-depend on it.
  - **A minimized window still tests fine — verified.** DOM, content scripts, `getComputedStyle`, and any injected overlay keep working while `_NET_WM_STATE … HIDDEN` (layout/CSS aren't gated on visibility). **Caveat:** `document.visibilityState` goes `"hidden"`, so `requestAnimationFrame`/timer logic is throttled — for rAF/timer-timing-sensitive tests, un-minimize briefly (or move-to-desktop) instead of minimizing. Everything else (overlay presence, colours, clicks) is safe minimized.
